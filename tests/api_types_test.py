...
def test_base_from_dict() -> None:
    assert Class1.from_dict({"a": 1, "b": 2}) == Class1(a=1, b=2, c=None)
    with pytest.raises(DataMappingError):
        Class1.from_dict({"b": 2, "c": "3"})


def test_base_handle_object() -> None:
    assert BaseTelegram._handle_object(Class1, {"a": 1, "c": "c"}) == Class1(
        a=1, b=None, c="c"
    )
    with pytest.raises(
        DataMappingError, match="Data without required keys: a"
    ):
...
def test_base_handle_field() -> None:
    ...
    assert BaseTelegram._handle_field(Class1, {"a": 1}) == Class1(
        a=1, b=None, c=None
    )

    union: Union[Any] = Union[Class1, Class2, None]
    assert BaseTelegram._handle_field(union, {"a": 1}) == Class1(
        a=1, b=None, c=None
    )
    assert BaseTelegram._handle_field(union, {"a": 1, "y": 0.1}) == Class2(
        a=1, y=0.1, z=None
    )
    message = (
        "Data \"{'a': 1, 'ooo': 0.1}\" not match any of "
        '"Class1, Class2, NoneType"'
    )
    with pytest.raises(DataMappingError, match=message):
        BaseTelegram._handle_field(union, {"a": 1, "ooo": 0.1})
...
def test_fixed_hints(
    _str: str, _type: api_types._Telegram, in_keys: bool, in_fields: bool
) -> None:
...
def test_input_media_serialization(type_: Any) -> None:
    ...
    with pytest.raises(
        TypeError,
        match="To serialize this object, the media "
